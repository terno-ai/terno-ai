...
class Agent:
    def __init__(self, llm=None, database_name: str = None):
        ...
        self.state = 'IDLE'
        self.tools = [DBSchema(database_name=database_name), GenerateSQL(), ExecuteSQL(database_name=database_name)]
        self.max_steps = 5
        self.current_step = 0
    ...
    def run(self, request: str) -> str:
        """
        Executes the agent's main loop to answer a user question.

        Args:
            request: The user's question about the database.

        Returns:
            A string containing the agent's response to the user's question.
        """
